import React from 'react';
import type { AiProfile } from '../../types';
import AiGeneratorCard from '../../components/AiGeneratorCard';
import DecodingMatrix from '../../components/DecodingMatrix';

const GridIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><line x1="3" y1="9" x2="21" y2="9"></line><line x1="3" y1="15" x2="21" y2="15"></line><line x1="9" y1="3" x2="9" y2="21"></line><line x1="15" y1="3" x2="15" y2="21"></line></svg>);

interface MatrixAiGeneratorProps {
    onGenerate: (aiProfile: AiProfile) => void;
    isLoading: boolean;
    isApplicable: boolean;
}

const MatrixAiGenerator: React.FC<MatrixAiGeneratorProps> = ({ onGenerate, isLoading, isApplicable }) => {
    return (
        <AiGeneratorCard 
            title="Matrix AI"
            description="Analista de Risco. Para Mines e Plinko, define o risco e a estratégia de cash out para lucros consistentes."
            icon={<GridIcon className="w-10 h-10 text-green-400"/>}
            onClick={() => onGenerate('Matrix')}
            disabled={isLoading || !isApplicable}
            disabledReason={!isApplicable ? 'Apenas para Matrix' : undefined}
            borderColor="border-green-500/50"
            contentTop={
                <DecodingMatrix themeColor="teal" />
            }
        />
    );
};

export default MatrixAiGenerator;
